"""Ensemble v5: igual que v4 pero con xG_proxy + ELO dinámico añadido.

Features v5 (43 total):
  - 9 odds: imp_h, imp_d, imp_a, avg_h, avg_d, avg_a, psc_h, psc_d, psc_a
  - 16 form: home/away n5/n10
  - 6 h2h: h2h5/10 home/draw/away
  - 3 rest: rest_days_home/away/diff
  - 6 xG_proxy: home/away/diff con n5 y n10
  - 3 elo: elo_home, elo_away, elo_diff

Walk-forward temporal, comparativa vs AvgH (mercado).
"""
...
PARQUET = ROOT / "data" / "training_set_v5.parquet"
...
FEATURE_COLS_V5 = [
    ...
    # xG_proxy
    "home_xg_proxy_n5", "away_xg_proxy_n5", "xg_proxy_diff_n5",
...
def build_training_set_v5() -> pd.DataFrame:
    """Une base v3 (odds+form+h2h+rest) con xG_proxy y ELO dinámico."""
    import sqlite3
    ...
    # xG_proxy (NUEVO v5)
    if (ROOT / "data" / "quiniela.db").exists():
        try:
            df = df.merge(pd.read_sql_query("""
                SELECT match_id,
                       home_xg_proxy_n5, away_xg_proxy_n5, xg_proxy_diff_n5,
            ...
            log.warning(f"match_xg_proxy no disponible: {e}")

    # ELO dinámico (NUEVO v5)
    try:
        df = df.merge(pd.read_sql_query(
...
def evaluate(df: pd.DataFrame, test_seasons: list[str]) -> dict:
    results = {}
    ...
    for ts in test_seasons:
        ...
        train = df[df["season"].isin(train_seasons)].dropna(subset=FEATURE_COLS_V5)
        test = df[df["season"] == ts].dropna(subset=FEATURE_COLS_V5)

        X_train = train[FEATURE_COLS_V5].values
        y_train = train["result"].values
        X_test = test[FEATURE_COLS_V5].values
...
def main():
    ...
    log.info("Construyendo training_set_v5...")
    df = build_training_set_v5()
    log.info(f"Loaded {df.shape[0]} partidos × {df.shape[1]} cols")
    if args.save_parquet:
        df.to_parquet(PARQUET)
        log.info(f"Guardado en {PARQUET}")

    n_with_xg = df["home_xg_proxy_n5"].notna().sum()
    n_with_elo = df["elo_home"].notna().sum()
    log.info(f"Con xG_proxy: {n_with_xg} ({100*n_with_xg/len(df):.1f}%)")
    log.info(f"Con ELO: {n_with_elo} ({100*n_with_elo/len(df):.1f}%)")

    results = evaluate(df, args.seasons)

    print("\n" + "=" * 80)
    print("RESUMEN WALK-FORWARD v5 (con xG_proxy + ELO dinámico)")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print("GO/NO-GO BINARIO v5 (vs AvgH)")
    print("=" * 80)
